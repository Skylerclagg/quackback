/**
 * CUTOVER PHASE 4 as a function — the audience-model translation and the
 * roadmap-membership rebuild that run AFTER the upstream migrator.
 *
 * Two callers share it: `04-transform.ts` (the laptop CLI in the runbook) and
 * `fork-cutover.ts` (the command the app image carries, so a Coolify deploy can
 * run the cutover without tooling on the host). It reads columns that exist
 ...
 */
// Relative rather than the workspace alias: bun resolves it from disk when run
// as a script and inlines it when bundled into the image.
import { createId, fromUuid, toUuid } from '../../packages/ids/src/index'
// oxlint-disable-next-line no-restricted-imports
import type { Sql } from 'postgres'

export async function runTransform(sql: Sql, apply: boolean): Promise<void> {
