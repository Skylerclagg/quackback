/**
 * Session audience scoping: a `widget` or `portal` session never satisfies a
 * team/permission gate, however the underlying principal's role changes
 * later. This is the structural backstop that lets /api/widget/identify mint
 * customer sessions for teammates (see identify-team-role-guard.test.ts).
 */
...
describe('requireAuth permission gates are dashboard-only', () => {
  ...
  it('treats a session row without a scope as dashboard (pre-migration rows)', async () => {
