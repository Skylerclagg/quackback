 ...
 *
 * Gated on AUTH_MANAGE (the admin tier): it reports directory addresses,
 * which is a wider set than the portal users a segment manager already
 * browses, so it keeps the fork's admin-only bar under upstream's RBAC.
 */
