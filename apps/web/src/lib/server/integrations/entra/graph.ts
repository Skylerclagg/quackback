...
export interface EntraDirectoryAccess {
  ...
  label: string
  /** Better-Auth providerId of the registration; `account.providerId` matches it. */
...
export async function resolveEntraDirectoryAccess(): Promise<EntraDirectoryAccess | null> {
  ...
  return {
    ...
    label: entra.label,
    registrationId: entra.registrationId,
  }
}
...
/**
 * Both caches are workspace-keyed, not module-global.
 *
 * Each workspace configures its own Entra app registration, so a shared token
 * cache would hand one tenant's Graph credentials to another, and a shared
 * roster cache would answer "who is in this group" with a different tenant's
 * directory. In a pooled process a module-scope Map survives the request and
 * therefore survives the workspace; policy/module-state enforces this.
 */
const groupEmailCache = new WorkspaceKeyedCache<CachedEmails>()
const tokenCache = new WorkspaceKeyedCache<{ token: string; expiresAt: number }>()

/** Sole key in {@link tokenCache} — one app-only token per workspace. */
...
async function getCachedGraphToken(access: EntraDirectoryAccess): Promise<string> {
  ...
  // policy's floor.
  tokenCache.set(TOKEN_CACHE_KEY, { token, expiresAt: Date.now() + 30 * 60 * 1000 })
  return token
}
