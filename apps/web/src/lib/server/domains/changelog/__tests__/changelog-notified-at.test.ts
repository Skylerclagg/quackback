...
describe('createChangelog wiring', () => {
  ...
  it('rejects empty or whitespace-only markdown without contentJson', async () => {
    const { createChangelog } = await import('../changelog.service')
  ...
  it('rejects an empty contentJson document or an empty bullet-list shell with empty markdown', async () => {
    const { createChangelog } = await import('../changelog.service')
  ...
  it('accepts empty markdown when contentJson has a list body', async () => {
    ...
    expect(mockInsertValues).toHaveBeenCalledWith(
      expect.objectContaining({ contentJson, content: expect.stringContaining('GIF per link') })
    )
  })

  it('accepts markdown-only content and image-only contentJson', async () => {
    const { createChangelog } = await import('../changelog.service')
    ...
    expect(mockInsertValues).toHaveBeenCalledWith(
      expect.objectContaining({ content: expect.stringContaining('Hello from markdown') })
    )
