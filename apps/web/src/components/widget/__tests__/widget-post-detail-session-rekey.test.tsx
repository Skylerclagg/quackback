...
/**
 * Upstream GH #464 follow-up: a first-visit upload/reaction/comment mints the
 * anonymous session mid-action, which bumps sessionVersion and re-keys the
 * post-detail query. The detail view must keep the current post (and so the
 * comment editors and reaction chips) mounted while the Bearer refetch runs;
 * the loading placeholder here would unmount the editor the in-flight upload
 * inserts into. Switching to a different post still shows the placeholder.
 */
...
/** The loading branch is a pulsing placeholder rather than a named skeleton. */
...
describe('WidgetPostDetail — session re-key keeps the view mounted', () => {
  ...
  it('still shows the loading placeholder when navigating to a different post', async () => {
    const { rerender } = render(<WidgetPostDetail postId="post_1" statuses={[]} />, { wrapper })
    await act(async () => pending[0].resolve(detail('post_1', 'First title')))
    await screen.findByText('First title')
