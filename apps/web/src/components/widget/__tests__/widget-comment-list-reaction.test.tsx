...
/**
 * Upstream GH #464 (same class): toggling a comment reaction can be a
 * first-time visitor's first write, so the widget must establish a session
 * before calling the reaction server fn — which requireAuth()s — or the
 * request goes out with no Bearer and is rejected silently.
 */
...
} as unknown as PublicCommentView

describe('WidgetCommentList — reaction session guard', () => {
  beforeEach(() => {
    session.token = null
    session.version = 0
    calls.length = 0
