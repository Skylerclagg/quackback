    ...
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        try {
          ...
          // Gated on the RBAC permission rather than a raw role check, so this
          // matches every other segment-management surface. A bare ['admin']
          // test would let a Manager edit segments everywhere else and refuse
          // them here.
          const held = await permissionsForPrincipal(
            validation.principal.id,
            validation.principal.role as Role
          )
          if (!held.has(PERMISSIONS.SEGMENT_MANAGE)) {
            log.warn({ role: validation.principal.role }, 'segment import access denied')
            return Response.json(
              { error: 'You do not have permission to manage segments' },
              { status: 403 }
            )
          }
